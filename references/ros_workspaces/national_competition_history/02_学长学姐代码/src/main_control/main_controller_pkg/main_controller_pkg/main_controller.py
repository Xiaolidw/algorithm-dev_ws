...
class MainController(Node):
    ...
    # 为单个任务分配最佳物块
    def assign_best_block_to_task(self, task, used_blocks):
        color = task["color"]
        target_area = task["to"]
    ...
    def _chat_callback_impl(self_ref, msg):
        ...
        try:
            ...
            # 1. 区分数据类型：如果是字典，按标识提取任务；如果是列表，直接作为任务
            if isinstance(parsed_data, dict):
                # 若为字典，需包含"type":"task"和"data"字段
                if parsed_data.get("type") != "task":
                    self.get_logger().info("忽略非任务字典消息（可能是其他数据）")
                    return
                task_json = parsed_data.get("data", [])
            elif isinstance(parsed_data, list):
                # 若为列表，直接视为任务列表（兼容parser未包装的情况）
                task_json = parsed_data
                self.get_logger().info("接收到列表格式任务数据，直接处理")
            else:
                # 既非字典也非列表，视为无效数据
                self.get_logger().info("忽略非列表/字典的JSON消息")
                return
            
            # 2. 确保task_json是列表（兼容单个任务字典的情况）
            ...
            # 解析失败的消息视为自然语言，直接忽略
            self.get_logger().info("忽略非JSON消息（可能是原始自然语言）")
    ...
    def _optimize_and_process_tasks(self):
        ...
        # 优化任务执行顺序
        self.optimized_tasks = self.optimize_task_order(current_tasks)
    ...
    def _process_next_optimized_task(self):
        ...
        self.get_logger().info(
            f"开始执行优化任务：{self.current_task['num']}个{color}物块 → {self.current_task['to']}区"
        )
        self.get_logger().info(
