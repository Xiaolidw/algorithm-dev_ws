...
class DynamicObstacle:
    ...
    def __init__(self, obstacle_name):
        ...
        self.is_moving = True
        self.position_updated = False  # 标记位置是否已更新
    ...
    def update_position(self, x, y, current_time, source="unknown"):
        """更新障碍物位置并计算速度"""
...
class MainController(Node):
    def __init__(self):
        ...
        self.watchdog_timer: Timer = None  # 位置更新监控定时器
        ...
        self.POSITION_TIMEOUT = 3.0  # 位置更新超时时间（秒）
        ...
        self.obstacle2_pose_sub = self.create_subscription(
            Pose, "/obstacle2/current_pose", lambda msg: self._obstacle_pose_callback("obstacle2", msg, "direct"), self.qos_reliable
        )
        self.obstacle3_pose_sub = self.create_subscription(
            Pose, "/obstacle3/current_pose", lambda msg: self._obstacle_pose_callback("obstacle3", msg, "direct"), self.qos_reliable
        )
        
        # 订阅gazebo/model_states作为备用方式
        self.model_states_sub = self.create_subscription(
            ModelStates, "/gazebo/model_states", self._model_states_callback, self.qos_best_effort
        )
        ...
        self.obstacle_status_pub = self.create_publisher(String, "/obstacle_status", self.qos_reliable)  # 发布障碍物状态
        self.foxglove_pubs = {
        ...
        self.watchdog_timer = self.create_timer(1.0, self._position_watchdog)  # 1秒检查一次位置更新
        self.status_publish_timer = self.create_timer(2.0, self._publish_obstacle_status)  # 2秒发布一次状态
        ...
        self.get_logger().info(f"  位置更新超时：{self.POSITION_TIMEOUT:.1f}s")
        self.get_logger().info("=" * 80)
        ...
        for name, obstacle in self.obstacles.items():
            if obstacle.start_pos and obstacle.end_pos:
                ...
                self.get_logger().info(f"    预设速度：{obstacle.speed}m/s")
        self.get_logger().info("=" * 80)
        self.get_logger().info("位置获取方式：")
        self.get_logger().info("  主要方式：订阅/obstacleX/current_pose话题")
        self.get_logger().info("  备用方式：订阅/gazebo/model_states话题")
        self.get_logger().info("  超时监控：1秒检查一次位置更新状态")
        self.get_logger().info("=" * 80)
    ...
    def _log_obstacle_distances(self):
        ...
        for name, obstacle in self.obstacles.items():
            ...
            status = "✅" if distance > safety_margin + 0.2 else "⚠️" if distance > safety_margin else "❌"
            self.get_logger().info(f"   {status} {name}：{distance:.2f}m (安全余量：{safety_margin:.2f}m)")

    # 障碍物位置回调（直接订阅方式）
    def _obstacle_pose_callback(self, obstacle_name, msg, source="direct"):
        ...
        if obstacle:
            ...
            obstacle.update_position(x, y, current_time, source)
            ...
            # 显示详细的位置更新信息
            self.get_logger().info(f"🚧 {obstacle_name}位置更新（来源：{source}）：")
        ...
        try:
            ...
            for i, name in enumerate(msg.name):
                if name in self.obstacles:
                    ...
                    # 只有当直接订阅方式没有更新时才使用备用方式
                    if not obstacle.position_updated or self.calculate_distance(obstacle.current_pos, (x, y)) > 0.1:
                        old_pos = obstacle.current_pos
                        obstacle.update_position(x, y, current_time, "model_states")
                        
                        position_change = self.calculate_distance((x, y), old_pos)
                        if position_change > 0.05:
                            self.get_logger().warn(f"🔄 使用备用方式更新{name}位置（model_states）：")
                            self.get_logger().warn(f"   旧位置：({old_pos[0]:.2f}, {old_pos[1]:.2f})")
                            self.get_logger().warn(f"   新位置：({x:.2f}, {y:.2f})")
                            self.get_logger().warn(f"   这可能表示直接订阅方式出现问题！")
                        
        except Exception as e:
            self.get_logger().error(f"更新模型状态时出错：{str(e)}")
    ...
    def _position_watchdog(self):
        ...
        for name, obstacle in self.obstacles.items():
            # 检查是否长时间没有更新
            if current_time - obstacle.last_update_time > self.POSITION_TIMEOUT:
                # 尝试使用初始位置
                if obstacle.start_pos and (obstacle.current_pos == (0.0, 0.0)):
                    self.get_logger().warn(f"⏰ {name}位置长时间未更新，使用初始位置：{obstacle.start_pos}")
                    obstacle.update_position(obstacle.start_pos[0], obstacle.start_pos[1], current_time, "initial")
                else:
                    self.get_logger().warn(f"⏰ {name}位置更新超时（{current_time - obstacle.last_update_time:.1f}秒）")
                    self.get_logger().warn(f"   最后已知位置：({obstacle.current_pos[0]:.2f}, {obstacle.current_pos[1]:.2f})")
                    self.get_logger().warn(f"   请检查auto_move_obstacle插件是否正常运行！")
            
            # 重置位置更新标记
            obstacle.position_updated = False

    # 发布障碍物状态
    def _publish_obstacle_status(self):
        """发布障碍物状态信息，供命令行工具查看"""
        status_info = {}
        for name, obstacle in self.obstacles.items():
            status_info[name] = {
                "position": obstacle.current_pos,
                "velocity": obstacle.get_speed(),
                "distance_to_robot": self.calculate_distance(self.current_robot_pose, obstacle.current_pos),
                "last_update_time": obstacle.last_update_time,
                "is_moving": obstacle.is_moving,
                "start_pos": obstacle.start_pos,
                "end_pos": obstacle.end_pos
            }
        
        status_msg = String()
        status_msg.data = json.dumps(status_info, indent=2)
    ...
    def _continuous_collision_check(self):
        ...
        if path_collision and not self.obstacle_waiting:
            ...
            if obstacle:
                ...
                self.get_logger().warn(f"   两者距离：{distance_to_robot:.2f}m")
    ...
    def destroy_node(self):
        ...
        if self.status_publish_timer:
            ...
            self.status_publish_timer.destroy()
