...
class MissionFlowExecutorNode(Node):
        ...
        if active or result in {'PENDING', 'ACCEPTED', 'CANCELING'}:
            ...
            if (active
                ...
                if (self.current_phase == 'DROPOFF'
                        and position_error <= self.dropoff_acceptance_m):
                    self.get_logger().info(
                        f'Drop-off zone accepted at {position_error:.3f} m '
                        'navigation residual; terminal yaw is not required '
                        'inside the placement area.')
                    if self.navigation_cancel_client.service_is_ready():
                        self.navigation_cancel_client.call_async(
                            Trigger.Request())
    ...
    def odom_callback(self, message):
        ...
        # A safe map detour can temporarily increase Nav2's
        # distance_remaining even while the chassis is moving normally.  Use
        # real odometry displacement as a second progress signal so the
        # watchdog only recovers a physically stuck robot, not a valid
        # obstacle-avoiding route.
        if (self.active and self.state in self.NAVIGATION_STATES
                and self.seen_current_navigation):
            current_xy = (self.latest_base_x, self.latest_base_y)
            if self.navigation_progress_anchor_xy is None:
                self.navigation_progress_anchor_xy = current_xy
            elif math.hypot(
                    current_xy[0] - self.navigation_progress_anchor_xy[0],
                    current_xy[1] - self.navigation_progress_anchor_xy[1]
            ) >= self.navigation_motion_progress_m:
                self.navigation_progress_anchor_xy = current_xy
    ...
    def is_expected_yaw_aligned(self):
    ...
    def watchdog_callback(self):
        ...
        if (self.active and self.state in self.NAVIGATION_STATES
            ...
            self.get_logger().warning(
                f'Navigation progress watchdog: neither path distance nor '
                f'chassis displacement advanced for {stalled_for:.1f}s; '
