...
class DynamicAvoidancePatrolTest(Node):
    ...
    def run(self):
        ...
        traversal_count = (
            self.arguments.traversals
            if self.arguments.traversals is not None
            else self.arguments.cycles * 2
        )
        self.get_logger().info(
            f'Traversals: {traversal_count}'
        )

        for traversal_index in range(1, traversal_count + 1):
            cycle_index = (traversal_index + 1) // 2
            going_to_b = traversal_index % 2 == 1
            self.get_logger().info(
                '================================'
            )
            self.get_logger().info(
                f'Traversal {traversal_index}/{traversal_count}'
            )

            target_name = 'B' if going_to_b else 'A'
            target = point_b if going_to_b else point_a
            target_yaw = yaw_a_to_b if going_to_b else yaw_b_to_a
            succeeded, status = (
                self.navigate_to(
                    target_name,
                    target[0],
            ...
            if not succeeded and not self.arguments.continue_on_failure:
                break
            if traversal_index < traversal_count:
                time.sleep(self.arguments.pause_seconds)
...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help='Exact number of alternating A/B crossings; overrides --cycles.',
    )
...
def main():
    ...
    if arguments.traversals is not None and arguments.traversals <= 0:
        raise ValueError(
            'traversals must be greater than zero.'
        )
