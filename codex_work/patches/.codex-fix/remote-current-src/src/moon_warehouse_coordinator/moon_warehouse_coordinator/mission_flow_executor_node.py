...
class MissionFlowExecutorNode(Node):
    ...
    def reset_runtime(self):
        ...
        self.coordinator_generation_started = False
        self.tasks = []
    ...
    def mission_status_callback(self, message):
        ...
        if self.active and mission_state in {
            'REQUESTING_QUESTION',
            'WAITING_SEMANTIC',
            'SEMANTIC_READY',
        }:
            # A transient-local subscription can replay the previous run's
            # PLAN_READY/plan immediately after reset.  Do not let those
            # retained samples start this run.  Seeing one of the states
            # above proves that the coordinator has started this generation.
            if mission_state == 'REQUESTING_QUESTION':
                self.latest_plan = None
                self.coordinator_plan_ready = False
            self.coordinator_generation_started = True

        if (
            self.active
            and self.state == 'WAITING_PLAN'
            and self.coordinator_generation_started
            and mission_state == 'PLAN_READY'
        ):
            self.coordinator_plan_ready = True
            self.try_begin_plan()

    def try_begin_plan(self):
        if not self.active or self.state != 'WAITING_PLAN':
            return
