...
void LSTMDynamicObstacleCritic::initialize()
{
  ...
  RCLCPP_INFO(
    node->get_logger(),
    "LSTMDynamicObstacleCritic: topic=%s safety=%.2f danger=%.2f weight=%.1f",
    prediction_topic_.c_str(), safety_radius_, danger_radius_, weight_);
}
...
void LSTMDynamicObstacleCritic::score(mppi::CriticData & data)
{
    ...
    step += static_cast<size_t>(trajectory_point_step_))
  {
    ...
    for (const auto & obstacle : prediction->trajectories) {
      ...
      if (found == obstacle.future_times.begin() || found == end) {
        const size_t index = found == end ? count - 1 : 0;
