...
class MissionFlowExecutorNode(Node):
    ...
    def navigation_status_callback(self, message):
        ...
        if active or result in {'PENDING', 'ACCEPTED', 'CANCELING'}:
            ...
            if (active
                    ...
                    and self.current_phase in ('PICKUP_PRE', 'PICKUP')):
                ...
                if (self.current_phase == 'PICKUP_PRE'
                    ...
                    def start_final_docking():
                        ...
                        if not self.active:
                            return
                        object_id = self.tasks[
                            self.current_task_index]['object_id']
                        target = self.object_dock_stages.get(
                            object_id, self.object_approaches[object_id])
                        phase = ('PICKUP_DOCK_STAGE'
                                 if object_id in self.object_dock_stages
                                 else 'PICKUP')
                        self.send_leg(phase, target, attempt=1)
    ...
    def start_precision_dock(self):
        ...
        self.start_precision_spin(
            generation,
            self.signed_angle_error(path_yaw, self.latest_base_yaw),
            'ALIGN_PATH',
        )
    ...
    def complete_precision_dock(self, generation):
        ...
        position_error = self.expected_position_error()
        yaw_aligned = self.is_expected_yaw_aligned()
        if (position_error > self.pickup_acceptance_m or not yaw_aligned):
            self.precision_dock_failed(
    ...
    def schedule_manipulation_retry(self, operation):
        """Re-run physical alignment after a transient pick/place failure."""
        self.cancel_retry_timer()
