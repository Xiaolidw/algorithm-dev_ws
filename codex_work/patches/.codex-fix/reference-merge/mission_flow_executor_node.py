...
class MissionFlowExecutorNode(Node):
    ...
    def start_post_place_departure(self):
        ...
        self.set_state(
            'DEPARTING_DROPOFF',
            f'Placed {task["object_id"]}; reversing clear before restoring '
            'it to the obstacle map',
            timeout_sec=8.0,
        )
        ...
        if cube_xy is not None:
            ...
            if cube_clearance >= self.post_place_min_cube_clearance_m:
                ...
                self.get_logger().info(
                    f'Placed cube already has {cube_clearance:.2f} m base '
                    'clearance; skipping blind reverse departure')
                self.finish_post_place_departure(0.0)
                return
    ...
    def post_place_departure_tick(self):
        ...
        if travelled >= self.post_place_departure_m:
            ...
            return
        if (self.post_place_departure_started_mono is not None
                and time.monotonic() - self.post_place_departure_started_mono
                > 7.0):
            self.publish_zero_velocity()
            task = self.tasks[self.current_task_index]
            cube_xy = self.cube_world_xy.get(task['object_id'])
    ...
    def finish_post_place_departure(self, travelled):
        ...
        self.set_state(
            'TASK_COMPLETED',
            f'Task {self.current_task_index + 1}/{len(self.tasks)} '
            f'completed; departed {travelled:.2f} m from placed cube',
        )
        self.current_task_index += 1
        if self.current_task_index >= len(self.tasks):
            self.complete_flow(
    ...
    def watchdog_callback(self):
        ...
        if (self.active and self.state in self.NAVIGATION_STATES
            ...
            self.get_logger().warning(
                f'Navigation remaining-distance watchdog: no 0.20 m '
                f'progress for {stalled_for:.1f}s; canceling and replanning '
