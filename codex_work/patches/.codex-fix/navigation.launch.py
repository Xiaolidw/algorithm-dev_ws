...
def generate_launch_description():
    ...
    gazebo_truth_localization = Node(
        package='moon_warehouse_bringup',
        executable='gazebo_truth_localizer.py',
        name='gazebo_truth_localizer',
        output='screen',
        parameters=[{'use_sim_time': use_sim_time}],
    )
    ...
    localization_recovery = TimerAction(
        period=12.0,
        actions=[ExecuteProcess(
            cmd=['bash', '-lc',
                 "for node in /map_server /amcl; do "
                 "state=$(ros2 lifecycle get $node 2>/dev/null | awk '{print $1}'); "
                 "if [ \"$state\" = unconfigured ]; then ros2 lifecycle set $node configure; fi; "
                 "state=$(ros2 lifecycle get $node 2>/dev/null | awk '{print $1}'); "
                 "if [ \"$state\" = inactive ]; then ros2 lifecycle set $node activate; fi; "
                 "done"],
            output='screen',
        )],
    )
