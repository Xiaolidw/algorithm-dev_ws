...
void LSTMDynamicObstacleCritic::initialize()
{
  ...
  get_param(exit_time_beta_, "exit_time_beta", exit_time_beta_);
  get_param(closing_speed_weight_, "closing_speed_weight", closing_speed_weight_);
  get_param(receding_discount_, "receding_discount", receding_discount_);
  get_param(relative_speed_scale_, "relative_speed_scale", relative_speed_scale_);
  ...
  exit_time_beta_ = std::max(0.0, exit_time_beta_);
  closing_speed_weight_ = std::max(0.0, closing_speed_weight_);
  receding_discount_ = std::clamp(receding_discount_, 0.0, 0.90);
  relative_speed_scale_ = std::max(0.05, relative_speed_scale_);
  ...
  RCLCPP_INFO(
    ...
    "risk_tube=%.2f exit_weight=%.2f beta=%.2f closing=%.2f weight=%.1f",
    prediction_topic_.c_str(), safety_radius_, danger_radius_,
    risk_tube_radius_, exit_time_weight_, exit_time_beta_,
    closing_speed_weight_, weight_);
}
...
void LSTMDynamicObstacleCritic::score(mppi::CriticData & data)
{
  ...
  struct PredictedPoint {double x; double y; double vx; double vy;};
  std::vector<std::vector<PredictedPoint>> obstacle_points(time_steps);
      ...
      if (found == obstacle.future_times.begin()) {
        ...
        if (count > 1) {
          const double dt = std::max(
            1e-6, static_cast<double>(obstacle.future_times[1] - obstacle.future_times[0]));
          vx = (obstacle.future_poses[1].position.x - pose.position.x) / dt;
          vy = (obstacle.future_poses[1].position.y - pose.position.y) / dt;
        }
        obstacle_points[step].push_back({pose.position.x, pose.position.y, vx, vy});
        continue;
      }
  ...
  for (size_t sample = 0; sample < trajectory_count; ++sample) {
      ...
      step += static_cast<size_t>(trajectory_point_step_))
    {
      ...
      const PredictedPoint * nearest_point = nullptr;
      for (const auto & point : obstacle_points[step]) {
        const double dx = static_cast<double>(tx(sample, step)) - point.x;
        const double dy = static_cast<double>(ty(sample, step)) - point.y;
      ...
      if (nearest_squared <= danger_squared) {
        ...
        if (nearest_point != nullptr && nearest > 1e-6 && step > 0) {
          const size_t previous_step = step >= static_cast<size_t>(trajectory_point_step_) ?
            step - static_cast<size_t>(trajectory_point_step_) : 0;
          const double sample_dt = std::max(
            1e-6, static_cast<double>(step - previous_step) * data.model_dt);
          const double robot_vx =
            (static_cast<double>(tx(sample, step)) -
            static_cast<double>(tx(sample, previous_step))) / sample_dt;
          const double robot_vy =
            (static_cast<double>(ty(sample, step)) -
            static_cast<double>(ty(sample, previous_step))) / sample_dt;
          const double dx = static_cast<double>(tx(sample, step)) - nearest_point->x;
          const double dy = static_cast<double>(ty(sample, step)) - nearest_point->y;
          const double closing_speed = -(
            dx * (robot_vx - nearest_point->vx) +
            dy * (robot_vy - nearest_point->vy)) / nearest;
          const double scaled = std::clamp(
            closing_speed / relative_speed_scale_, -1.0, 1.0);
          direction_factor = scaled >= 0.0 ?
            1.0 + closing_speed_weight_ * scaled :
            1.0 + receding_discount_ * scaled;
        }
        accumulated += direction_factor * normalized * normalized *
          std::exp(-0.5 * future_time);
      }
    ...
    if (!collision && risk_seen) {
      ...
      const double exit_penalty = exit_time_beta_ > 1e-6 ?
        std::expm1(exit_time_beta_ * normalized_exit) /
          std::expm1(exit_time_beta_) : normalized_exit;
      trajectory_cost += exit_time_weight_ * exit_penalty;
    }
