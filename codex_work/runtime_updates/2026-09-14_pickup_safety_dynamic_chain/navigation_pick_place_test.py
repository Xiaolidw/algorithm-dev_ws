...
class PickPlaceTest(Node):
    ...
    def navigate_dropoff_transits(self):
        ...
        if self.destination == 'C':
            if self.object_id == 'red_cube_4':
                ...
                for index, transit in enumerate(rail_exit, 1):
                    self.publish_navigation_status(
                        ...
                        route_mode='red4_to_c_west_rail_exit')
                    self.navigate(
                        f'red_cube_4 to-C rail exit {index}', transit,
            ...
            if obstacle_y > -4.5:
                bypass_x = 0.35
                route_mode = 'c_wait_then_north_crossing'
            else:
                bypass_x = 0.35
                route_mode = 'c_immediate_north_crossing'
            staging = (
    ...
    def navigate_c_post_place_exit(self):
        ...
        for index, transit in enumerate(transits, 1):
            ...
            self.publish_navigation_status(
                ...
                transit_index=index, transit_count=len(transits),
                route_mode='c_guarded_north_exit')
            self.navigate(
    ...
    def compute_dropoff_target(self, configured_target):
        ...
        if self.destination in ('A', 'B', 'C'):
            ...
            if self.destination == 'A':
                ...
                # above the 0.05 m pair-clearance gate.  The carried-object
                # offset varies with grasp yaw: blue_cube_5 landed 0.144 m
                # farther east than its nominal x and missed the true 0.475 m
                # boundary by 9 mm.  A repeat with the 40 mm inset still
                # drifted to x=0.506 only after the arm returned home, proving
                # the worst post-release sweep is about 0.20 m.  Use x=0.18
                # so that sweep still finishes with roughly 0.11 m of margin.
                candidates = [
