...
class CoStudioMissionGateway(Node):
    def __init__(self):
        ...
        self.llm_command_pub = self.create_publisher(
            String, '/user/text_command', 10)
        self.create_subscription(
            String, '/costudio/mission_request', self.request_callback, 10)
        self.create_subscription(
            String, '/llm/task_plan_json', self.task_plan_callback, 10)
        self.create_subscription(
    ...
    def request_callback(self, message):
        ...
        try:
            ...
            self.publish_state('WAITING_SEMANTIC', 'DeepSeek 正在解析题目与映射')
            self.question_pub.publish(String(data=question))
            mapping_text = json.dumps(
                {'mappings': normalized}, ensure_ascii=False)
            combined = (
                '【比赛题目】\n' + question
                + '\n\n【本场动态映射规则】\n' + mapping_text)
            self.llm_command_pub.publish(String(data=combined))
    ...
    def task_plan_callback(self, message):
        ...
        try:
            ...
            if data.get('action') != 'execute':
                reason = str(data.get('reason', '模型未生成执行计划'))
                raise ValueError(f'大模型拒绝或需要澄清：{reason}')
            steps = data.get('steps')
            if not isinstance(steps, list) or not steps:
                raise ValueError('大模型未返回有效 steps')
            allowed_pairs = {
                (row['color'], row['destination'])
                for row in self.pending['mapping']
            }
            tasks = []
            for step in steps:
                if not isinstance(step, dict):
                    raise ValueError('大模型 step 必须是对象')
                color = str(step.get('color', '')).strip().lower()
                destination = str(
                    step.get('destination', '')).strip().upper()
                count = step.get('count')
                if (color, destination) not in allowed_pairs:
                    raise ValueError(
                        f'计划映射不在本场规则中：{color}->{destination}')
                if isinstance(count, bool) or not isinstance(count, int) or count < 0:
                    raise ValueError('每步 count 必须是非负整数')
                for _ in range(count):
                    tasks.append({
                        'sequence': len(tasks) + 1,
                        'object_id': f'fastest-{color}',
                        'color': color,
                        'destination': destination,
                        'status': 'PENDING',
            ...
            # Match the validated runner policy before publishing the plan so
            # CoStudio's current-target/progress cards describe the actual
            # execution order.  Stable sorting preserves the LLM order within
            # one destination; C remains last to avoid an extra guarded exit.
            zone_rank = {'B': 0, 'A': 1, 'C': 2}
            tasks.sort(key=lambda task: zone_rank[task['destination']])
            ...
            self.publish_state(
                'PLAN_READY',
                f'DeepSeek 解析成功，生成 {len(tasks)} 个任务',
                llm_task_id=str(data.get('task_id', '')),
