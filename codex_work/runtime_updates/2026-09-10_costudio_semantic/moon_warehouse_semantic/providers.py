"""OpenAI-compatible model providers for semantic parsing."""
...
class DeepSeekProvider:
    ...
    def solve(self, question: str) -> SolverResult:
        ...
        if response.status_code >= 400:
            ...
            raise ProviderError(
                f'Model returned HTTP {response.status_code}: {detail}'
            )
        ...
        return SolverResult(
            ...
            provider='local_llama' if self.is_local else 'deepseek',
            model=self.model,
            raw_text=raw_text,
        )
