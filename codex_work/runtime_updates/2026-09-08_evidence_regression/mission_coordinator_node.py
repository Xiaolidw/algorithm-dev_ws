...
class MissionCoordinatorNode(Node):
    ...
    def model_states_callback(self, message):
        """Cache lightweight Gazebo truth for nearest-task assignment."""
    ...
    def build_task_queue(self, variables):
        ...
        for variable_name, rule in self.task_mapping.items():
            ...
            requirements.append({
                ...
                'remaining': int(requested_count),
                'objects': [str(item) for item in object_ids],
            })

        # Greedy live assignment: after every conceptual delivery the robot is
        # expected at that destination, so choose the nearest still-required
        # cube from there.  This jointly orders colours and selects cube IDs;
        # YAML list order is only a deterministic fallback when Gazebo truth
        # has not arrived yet.
        robot_pose = self.live_model_poses.get('six_arm')
        current_x = float(robot_pose.position.x) if robot_pose else 0.0
        current_y = float(robot_pose.position.y) if robot_pose else 0.0
        used = set()
        task_queue = []
        sequence = 1
        zone_models = {'A': 'zone_a', 'B': 'zone_b', 'C': 'zone_c'}

        while any(item['remaining'] > 0 for item in requirements):
            ranked = []
            for requirement in requirements:
                if requirement['remaining'] <= 0:
                    continue
                for order, object_id in enumerate(requirement['objects']):
                    if object_id in used:
                        continue
                    pose = self.live_model_poses.get(object_id)
                    if pose is None:
                        distance = 1.0e6 + float(order)
                    else:
                        distance = math.hypot(
                            float(pose.position.x) - current_x,
                            float(pose.position.y) - current_y,
                        )
                    ranked.append((distance, order, object_id, requirement))

            if not ranked:
                raise ValueError('no unused objects remain for task request')
            distance, _, object_id, requirement = min(
            ...
            task_queue.append({
                ...
                'selection_distance_m': (
                    None if distance >= 1.0e6 else round(distance, 3)),
            })
            used.add(object_id)
            requirement['remaining'] -= 1
            sequence += 1

            zone_name = zone_models[requirement['destination']]
            zone_pose = self.live_model_poses.get(zone_name)
            if zone_pose is not None:
                current_x = float(zone_pose.position.x)
                current_y = float(zone_pose.position.y)
