...
void GazeboLinkAttacherPrivate::Detach(
  ...
  linkattacher_msgs::srv::DetachLink::Response::SharedPtr _res)
{
  // Remove the exact matching record as part of detach.  The original code
  // left detached JointPtr entries in GV_joints, so the next cube with the
  // same robot/tool pair could match a stale joint and dereference links that
  // Gazebo had already destroyed.
  for (auto it = GV_joints.begin(); it != GV_joints.end(); ++it) {
    if (it->model1 != _req->model1_name ||
        it->model2 != _req->model2_name ||
        it->link1 != _req->link1_name ||
        it->link2 != _req->link2_name) {
      continue;
    }
    ...
    if (!model1 || !it->joint) {
      GV_joints.erase(it);
      IsAttached = !GV_joints.empty();
      _res->success = false;
      _res->message = "DETACHED -- ERROR (stale model or joint record).";
      return;
    }

    const std::string joint_name = it->joint->GetName();
    it->joint->Detach();
    model1->RemoveJoint(joint_name);
    GV_joints.erase(it);
    IsAttached = !GV_joints.empty();
