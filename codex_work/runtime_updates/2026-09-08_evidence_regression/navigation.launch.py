...
def generate_launch_description():
    ...
    initial_pose = TimerAction(
        ...
        actions=[Node(
            ...
            parameters=[{
                # This bootstrap node must not wait indefinitely when Fast DDS
                # delivers /clock late during a loaded Gazebo startup.  The
                # message itself uses a zero stamp and therefore does not need
                # simulation time.
                'use_sim_time': False,
                'x': initial_x,
                'y': initial_y,
                'yaw': initial_yaw,
                # Cover delayed lifecycle activation on the VM.  AMCL may not
                # be ready during the first short burst.
                'publish_count': 45,
            }],
        )],
    )
