...
class PickPlaceTest(Node):
    ...
    def navigate(self, label, target, handoff_distance=None, lock_route=False):
        """
    ...
    def _wait_navigation_result(
        ...
        threshold = max(0.05, float(handoff_distance))
        ...
        while rclpy.ok() and not result_future.done():
            ...
            self.get_logger().info(
                f'Pickup handoff reached: distance={distance:.3f}m, '
                f'threshold={dynamic_threshold:.3f}m; canceling Nav2 before fine dock')
            self.publish_navigation_status(
