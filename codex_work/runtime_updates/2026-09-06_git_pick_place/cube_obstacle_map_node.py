...
class CubeObstacleMapNode(Node):
    ...
    def __init__(self):
        ...
        # This restored layer is deliberately cube-only. Static stones and
        # walls remain exclusively owned by the existing competition map.
        self.declare_parameter('fixed_obstacles', [])
        # 放置区(A/B/C 标记牌 1.0 x 0.5m): 整区视为障碍, 返回/经过时
        # 全局路径会绕开, 不会碾压区内已放置的物块。当前任务的目的地
        # 区域临时放行, 保证本任务可以靠近放置。
        self.declare_parameter('zone_areas', [])
    ...
    def pick_place_status_callback(self, message):
        """Apply the same target exclusion to the focused regression runner."""
        try:
            payload = json.loads(message.data)
        ...
        if phase == 'COMPLETE' and object_id in self.cube_positions:
            self.transient_excluded_cubes.add(object_id)
            active = set()
        elif phase == 'FAILED':
            active = set()
        else:
            active = ({object_id}
                      if object_id in self.cube_positions else set())
        if active == self.active_target_cubes and event != 'acceptance_passed':
            return
        self.active_target_cubes = active
        self.refresh_mission_exclusions()
        self.grid = self.build_grid()
        self.publish_map()
