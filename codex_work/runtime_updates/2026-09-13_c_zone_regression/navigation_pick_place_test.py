...
class PickPlaceTest(Node):
    ...
    def select_object(self, selector, approaches):
        ...
        for object_id in candidates:
            ...
            if object_id in ('red_cube_4', 'red_cube_5'):
                # Both upper cubes require the immutable west-rail bypass.
    ...
    def navigate_pickup_transits(self):
        ...
        if leaving_a and self.destination == 'B' and self.object_id.startswith('blue_cube_'):
        ...
        if self.object_id == 'red_cube_5':
            ...
            # rail. The apparent east shortcut is not connected in the static
            # map (A* exhausted its iterations at x=1.75,y=2.00), so reuse the
            # physically verified west-end crossing used for red_cube_4.
            transits = (
            ...
            for index, transit in enumerate(transits, 1):
                self.publish_navigation_status(
                    ...
                    route_mode='red5_pickup_west_rail_bypass')
                self.navigate(
                    f'red_cube_5 pickup west rail bypass {index}', transit,
    ...
    def navigate_dropoff_transits(self):
        ...
        if self.destination == 'C':
            # The new C route crosses the vertical rail of moving_obstacle_2
            # at x=2, y=-3..-6 and then passes below the fixed wall endpoint.
            # Both homotopies are statically free.  Pick one once from fresh
            # Gazebo truth; never switch sides in motion, which would provoke
            # replanning oscillation.  SIPP and Collision Monitor retain
            # ownership of the final time separation from the obstacle.
            obstacle = self.model_poses.get('moving_obstacle_2')
            if obstacle is None:
                raise RuntimeError(
                    'moving_obstacle_2 pose unavailable for C route choice')
            obstacle_y = float(obstacle.position.y)
            if obstacle_y > -4.5:
                # Wide west lane: 1.65 m centre separation from the rail.
                bypass_x = 0.35
                route_mode = 'c_west_bypass_obstacle_upper'
                transits = (
                    # Wait spatially west of an obstacle occupying the upper
                    # half, then cross beyond its north endpoint into the
                    # already-proven east lane.  Do not cut diagonally around
                    # the fixed wall's south end: that failed at
                    # (2.884,-6.010) in the west-branch trace.
                    {'x': 0.35, 'y': -1.00, 'yaw': 0.0},
                    {'x': 2.85, 'y': -1.00, 'yaw': 0.0},
                    # Establish full lateral clearance before travelling
                    # south.  The former diagonal from x=2.85 skimmed the
                    # moving rail/fixed-wall corner and produced a 6.74 m/s
                    # impact during the four-cube C acceptance run.
                    {'x': 3.75, 'y': -1.00, 'yaw': -math.pi / 2.0},
                    {'x': 3.75, 'y': -6.65, 'yaw': 0.0},
                )
            else:
                # The east lane is narrower, so place both Nav2 goals beyond
                # the rail endpoints.  The earlier y=-2.55 goal became lethal
                # when the obstacle reversed before the robot arrived.
                bypass_x = 2.85
                route_mode = 'c_east_bypass_obstacle_lower'
                transits = (
                    {'x': 2.85, 'y': -1.00, 'yaw': 0.0},
                    {'x': 3.75, 'y': -1.00, 'yaw': -math.pi / 2.0},
                    {'x': 3.75, 'y': -6.65, 'yaw': 0.0},
            ...
            self.get_logger().info(
                ...
                f'{route_mode}, bypass_x={bypass_x:.2f}')
            for index, transit in enumerate(transits, 1):
                self.publish_navigation_status(
    ...
    def compute_dropoff_target(self, configured_target):
        ...
        if self.destination in ('A', 'B', 'C'):
            ...
            if self.destination == 'A':
                # Keep every dock on one open-side parking line.  Mixing local
                # x rows made later docks cross the inflation of cubes already
                # stored in the deeper row, leaving only three reachable
                # slots.  Four 0.09 m-spaced centres fit the short axis, remain
                # above the 0.05 m pair-clearance gate, and x=0.34 leaves the
                # observed ~0.08 m arm sweep inside |x|<=0.475.
                candidates = [
                    (0.34, -0.18), (0.34, -0.09), (0.34, 0.00),
                    (0.34, 0.09),
                ]
                ...
                # C is approached only from its open north side.  A compact
                # 2x2 interior grid keeps the two rows 0.15 m apart and the
                # two columns 0.32 m apart.  The north row is intentionally
                # inset to +0.05: the manipulation server's actual y limit is
                # 0.225 m after its boundary margin, while the bounded dock
                # may legally hand off with 0.15 m residual position error.
                # Two parallel north-side chassis docks preserve the stable
                # straight-arm geometry.  Unlike the former centre-only rule this
                # supports any four of the five blue cubes without overlap.
                candidates = [
                    (-0.16, -0.10), (0.16, -0.10),
                    (-0.16, 0.05), (0.16, 0.05),
                ]
            preferred_indices = {
                'A': {4: 0, 3: 1, 2: 2, 5: 3, 1: 3},
