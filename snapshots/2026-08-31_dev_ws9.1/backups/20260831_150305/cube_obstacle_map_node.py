...
class CubeObstacleMapNode(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('robot_model_name', 'originbot')
        ...
        self.declare_parameter(
            ...
            ['zone_a:-11.0:-5.8', 'zone_b:-1.0:-2.5',
             'zone_c:11.0:-2.2'])
    ...
    def execution_status_callback(self, message):
        ...
        completion_key = (task_index, object_id)
        if (
            state == 'TASK_COMPLETED'
            and object_id in self.cube_positions
            and completion_key not in self.completed_exclusion_keys
        ):
            self.completed_exclusion_keys.add(completion_key)
            cube_xy = self.cube_positions.get(object_id)
            clearance = (
                math.hypot(
                    self.robot_xy[0] - cube_xy[0],
                    self.robot_xy[1] - cube_xy[1],
            ...
            if clearance < self.placed_cube_release_clearance:
                ...
                self.get_logger().info(
                    f'Holding {object_id} out of the virtual layer at '
