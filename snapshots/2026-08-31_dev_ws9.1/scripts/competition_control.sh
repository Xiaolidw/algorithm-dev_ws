...
DASHBOARD_PID_FILE="${RUNTIME_DIR}/moon-warehouse-dashboard.pid"
...
open_dashboard() {
  ...
  if command -v gnome-terminal >/dev/null 2>&1; then
    gnome-terminal --title='赛题系统状态面板' -- bash -lc \
      "echo \$\$ > '${DASHBOARD_PID_FILE}'; trap 'rm -f \"${DASHBOARD_PID_FILE}\"' EXIT; /home/ros/dev_ws/scripts/competition_live_dashboard.sh; exec bash" \
      >/dev/null 2>&1 &
  fi
}
