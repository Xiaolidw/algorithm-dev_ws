...
class MissionFlowExecutorNode(Node):
    ...
    def start_post_place_departure(self):
        task = self.tasks[self.current_task_index]
        self.cancel_post_place_departure_timer()
        if self.latest_base_x is None or self.latest_base_y is None:
    ...
    def cancel_post_place_departure_timer(self):
        """Cancel without destroying a timer that may own this callback."""
        timer = self.post_place_departure_timer
