                    ...
                    if (robot) {
                        ...
                        if (this->stop_distance <= 0.0) {
                            ...
                            if (!this->yielding_to_robot &&
                                ...
                                this->yielding_to_robot = false;
                                this->yield_wait_for_robot_pass = true;
                            } else if (this->yield_wait_for_robot_pass &&
                                       entering_yield) {
                                // A robot that turns back before passing keeps
                                // the original retreat direction; it may never
                                // make the obstacle reverse toward it.
                                this->yield_wait_for_robot_pass = false;
                                this->yielding_to_robot = true;
                            }
                        } else {
                        ...
                        if (this->yielding_to_robot) {
                            ...
                            if (robot_distance <=
                                    this->emergency_stop_distance ||
                                !route_has_room || control_dt <= 0.0) {
