...
class LightweightSippDecision(Node):
    def __init__(self):
        ...
        self.detour_min_wait_time = max(
            0.0, float(self.detour_min_wait_time))
        self.detour_base_extra_time = max(
            0.0, float(self.detour_base_extra_time))
        self.detour_risk_time_weight = max(
            0.0, float(self.detour_risk_time_weight))
        self.detour_safe_trajectory_ratio = min(
            1.0, max(0.0, float(self.detour_safe_trajectory_ratio)))
        self.detour_min_collision_ttc = max(
            0.0, float(self.detour_min_collision_ttc))
        self.detour_speed = max(0.05, float(self.detour_speed))
        self.detour_min_commit_time = max(
            0.0, float(self.detour_min_commit_time))
        self.detour_recheck_confirm_cycles = max(
            1, int(self.detour_recheck_confirm_cycles))
        self.release_confirm_time = max(
            0.0, float(self.release_confirm_time))
        self.pass_release_max_nav_angular_speed = max(
            0.0, float(self.pass_release_max_nav_angular_speed))
        self.pass_emergency_ttc = max(
            0.0, float(self.pass_emergency_ttc))
    ...
    def track_details(self, track):
        ...
        keys = (
            ...
            'decision_reason',
            'expected_wait_time', 'detour_safe_trajectory_ratio',
            'detour_earliest_collision_ttc', 'detour_estimated_extra_time',
            'robot_entry_from_current',
    ...
    def select_speed_mode(
        ...
        if state == 'DETOUR_COMMITTED':
            return 'DETOUR', float(self.detour_speed), future_conflict
        if state in {
            'CRUISE', 'PASS_COMMITTED', 'DETOUR_COMMITTED',
            'APPROACH_STOP_LINE',
    ...
    def fallback_from_pass(self, track, snapshot, reason):
        ...
        self.pass_stall_since = None
        if self.pass_before_entry(track, snapshot):
            # The original stop line may be behind while the hard conflict is
            # still ahead. Recompute the choice from the current pose instead
            # of blindly preserving PASS or blindly waiting.
            entry_s = float(track.get(
                'hard_conflict_s_enter', track['conflict_s_enter']))
            entry_progress = entry_s - float(track['conflict_s_center'])
            distance_to_entry = max(
                0.0,
                entry_progress - self.track_forward_progress(track, snapshot),
            )
            robot_entry_time = self.travel_time_for_distance(
                distance_to_entry, abs(self.linear_speed))
            obstacle_exit_time = float(track.get(
                'obstacle_hard_t_exit', track.get('obstacle_t_exit', 0.0)))
            remaining_wait = max(
                0.0,
                obstacle_exit_time + float(self.release_time_margin)
                - robot_entry_time,
            )
            track['robot_entry_from_current'] = robot_entry_time
            track['expected_wait_time'] = remaining_wait
            if self.waiting_detour_is_better(
                track, snapshot, remaining_wait,
                expected_state=str(track.get('status', 'PASS_COMMITTED')),
            ):
                track['status'] = 'DETOUR_COMMITTED'
                self.arm_pass_commit(track, snapshot)
                self.publish_track(
                    'DETOUR_COMMITTED', track, snapshot,
                    f'{reason}_redecision_prefers_detour')
            else:
                ...
                self.publish_track(
                    ...
                    f'{reason}_redecision_prefers_wait', True)
        elif distance_to_stop >= -float(self.stop_reached_tolerance):
    ...
    def pass_execution_ready(self, snapshot):
        ...
        MPPI runs upstream of this gate, so its raw command remains observable
        while PREPARE_TO_PASS holds the executed velocity at zero.  Require a
        meaningful forward candidate: releasing on a near-zero MPPI command
        strands the robot in the crossing until the obstacle's next pass.
        """
        command_fresh = (
        ...
        if (
            not command_fresh
            or self.nav_cmd_linear < float(self.pass_release_min_nav_speed)
            or abs(self.nav_cmd_angular)
            > float(self.pass_release_max_nav_angular_speed)
        ):
            return False
    ...
    def waiting_detour_is_better(
        self, track, snapshot, remaining_wait,
        expected_state='WAIT_AT_STOP_LINE',
    ):
        """Reconsider a local detour after MPPI has sampled from a full stop."""
        if (
            not bool(self.enable_local_detour)
            or not math.isfinite(remaining_wait)
            or remaining_wait < float(self.detour_min_wait_time)
        ):
            return False
        feedback = self.valid_mppi_feedback(
            snapshot['now'], expected_state=expected_state)
        if feedback is None:
            return False
        status = str(feedback.get('status', 'UNKNOWN'))
        safe_ratio = float(feedback.get('safe_trajectory_ratio', 0.0))
        risk_ratio = min(1.0, max(
            0.0, float(feedback.get('risk_trajectory_ratio', 1.0))))
        collision_ttc = float(feedback.get('earliest_collision_ttc', -1.0))
        extra_time = (
            float(self.detour_base_extra_time)
            + float(self.detour_risk_time_weight) * risk_ratio)
        track['expected_wait_time'] = remaining_wait
        track['detour_safe_trajectory_ratio'] = safe_ratio
        track['detour_earliest_collision_ttc'] = collision_ttc
        track['detour_estimated_extra_time'] = extra_time
        return (
            status in {'ACCEPTED', 'CLEAR'}
            and safe_ratio >= float(self.detour_safe_trajectory_ratio)
            and remaining_wait > extra_time
            and (
                collision_ttc < 0.0
                or collision_ttc >= float(self.detour_min_collision_ttc))
            and not bool(feedback.get('override_recommended', False))
        )
    ...
    def evaluate_active_track(self, snapshot):
        ...
        if status in {'PASS_COMMITTED', 'DETOUR_COMMITTED'}:
            committed_state = status
            feedback = self.valid_mppi_feedback(
                snapshot['now'], expected_state=committed_state)
            ...
            if feedback is not None:
                feedback_blocks = (
                    ...
                    < float(self.mppi_pass_min_safe_ratio))
            timing_blocks = (
                committed_state == 'PASS_COMMITTED'
                and
                matched is not None
            ...
            hard_feedback_emergency = False
            if emergency_feedback is not None:
                feedback_ttc = float(
                    emergency_feedback.get('earliest_collision_ttc', -1.0))
                feedback_clearance = float(
                    emergency_feedback.get('min_clearance', math.inf))
                hard_feedback_emergency = (
                    bool(emergency_feedback.get('override_recommended', False))
                    or (
                        feedback_ttc >= 0.0
                        and feedback_ttc <= float(self.pass_emergency_ttc)
                        and feedback_clearance >= 0.0
                        and feedback_clearance <= float(self.hard_release_radius)
                    )
                )
            if hard_feedback_emergency:
                track['pass_block_count'] = 0
                self.pass_stall_since = None
                self.publish_track(
            ...
            commit_age = (
                snapshot['now']
                - float(track.get('pass_committed_time', snapshot['now'])))
            block_threshold = (
                self.detour_recheck_confirm_cycles
                if committed_state == 'DETOUR_COMMITTED'
                else self.pass_recheck_confirm_cycles)
            commitment_can_be_reconsidered = (
                committed_state != 'DETOUR_COMMITTED'
                or commit_age >= float(self.detour_min_commit_time))
            ...
            if (
                before_entry
                and commitment_can_be_reconsidered
            ...
            progress_timeout = float(self.pass_stall_timeout)
            if committed_state == 'DETOUR_COMMITTED':
                progress_timeout = max(
                    progress_timeout, float(self.detour_min_commit_time))
            if (
                before_entry
            ...
            if stalled:
                ...
                self.pass_stall_since = None
            self.publish_track(
                committed_state, track, snapshot, 'conflict_locked')
            return True
        ...
        if status == 'APPROACH_STOP_LINE':
            # Feedback is often unavailable at first detection but becomes
            # valid while approaching the stop line. Re-evaluate now so a
            # feasible detour does not require a complete stop first.
            approach_wait = max(0.0, safe_delay)
            if self.waiting_detour_is_better(
                track, snapshot, approach_wait,
                expected_state='APPROACH_STOP_LINE',
            ):
                track['status'] = 'DETOUR_COMMITTED'
                track['safe_since'] = None
                track['prepare_angular_z'] = 0.0
                ...
                self.publish_track(
                    'DETOUR_COMMITTED', track, snapshot,
                    'approach_redecision_prefers_detour')
                return True
            if distance_to_stop <= float(self.stop_reached_tolerance):
                track['status'] = 'WAIT_AT_STOP_LINE'
                self.publish_track(
        ...
        if status == 'WAIT_AT_STOP_LINE':
            if self.waiting_detour_is_better(track, snapshot, safe_delay):
                track['status'] = 'DETOUR_COMMITTED'
                track['safe_since'] = None
                track['prepare_angular_z'] = 0.0
                self.arm_pass_commit(track, snapshot)
                self.publish_track(
                    'DETOUR_COMMITTED', track, snapshot,
                    'waiting_redecision_prefers_detour')
                return True
            if safe_now:
        ...
        if status == 'PREPARE_TO_PASS':
            ...
            safe_stable = (
                track.get('safe_since') is not None
                and snapshot['now'] - track['safe_since']
                >= float(self.release_confirm_time))
    ...
    def evaluate(self):
        ...
        feedback = self.valid_mppi_feedback(snapshot['now'])
        expected_wait = 0.0
        if candidate.get('hard_occupied', True):
            expected_wait = max(
                0.0,
                candidate['obstacle_hard_t_exit']
                + float(self.release_time_margin)
                - candidate['robot_hard_t_enter'],
            )
        candidate['expected_wait_time'] = expected_wait
        if (
            relation == 'STOP'
            and bool(self.enable_local_detour)
            and expected_wait >= float(self.detour_min_wait_time)
            and feedback is not None
        ):
            feedback_status = str(feedback.get('status', 'UNKNOWN'))
            safe_ratio = float(feedback.get('safe_trajectory_ratio', 0.0))
            risk_ratio = float(feedback.get('risk_trajectory_ratio', 1.0))
            collision_ttc = float(feedback.get('earliest_collision_ttc', -1.0))
            detour_extra_time = (
                float(self.detour_base_extra_time)
                + float(self.detour_risk_time_weight)
                * min(1.0, max(0.0, risk_ratio))
            )
            candidate['detour_safe_trajectory_ratio'] = safe_ratio
            candidate['detour_earliest_collision_ttc'] = collision_ttc
            candidate['detour_estimated_extra_time'] = detour_extra_time
            ttc_allows_detour = (
                collision_ttc < 0.0
                or collision_ttc >= float(self.detour_min_collision_ttc)
            )
            if (
                feedback_status in {'ACCEPTED', 'CLEAR'}
                and safe_ratio >= float(self.detour_safe_trajectory_ratio)
                and expected_wait > detour_extra_time
                and ttc_allows_detour
                and not bool(feedback.get('override_recommended', False))
            ):
                relation = 'DETOUR'
                relation_reason = 'detour_faster_than_wait_and_mppi_feasible'
        ...
        if relation in {'PASS', 'DETOUR'} and snapshot['now'] < self.mppi_block_until:
            relation = 'STOP'
        ...
        elif track['status'] == 'DETOUR_COMMITTED':
            self.publish_track(
                'DETOUR_COMMITTED', track, snapshot, relation_reason)
