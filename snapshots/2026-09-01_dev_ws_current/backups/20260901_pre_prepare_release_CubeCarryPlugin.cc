...
public:
            ...
            sdf::ElementPtr sdf) override
  {
    ...
    base_lock_sub_ = node_->create_subscription<std_msgs::msg::Bool>(
      "/manipulation/base_lock", qos,
      [this](const std_msgs::msg::Bool::SharedPtr msg) {
        if (msg->data && !base_locked_) {
          auto robot = world_->ModelByName("six_arm");
          if (robot) {
            locked_base_pose_ = robot->WorldPose();
            base_locked_ = true;
            gzmsg << "[cube_carry] manipulation base lock engaged\n";
          }
        } else if (!msg->data && base_locked_) {
          base_locked_ = false;
          gzmsg << "[cube_carry] manipulation base lock released\n";
        }
      });
  ...
  void OnUpdate(const gazebo::common::UpdateInfo &)
  {
    ...
    if (base_locked_) {
      // Manipulation is stationary work.  Holding all chassis DOFs here
      // prevents arm reaction/contact forces from invalidating the measured
      // cube-to-base transform; normal wheel physics resumes immediately on
    ...
    if (carried_.empty()) {
      if (!release_guard_cube_.empty()) {
        auto released = world_->ModelByName(release_guard_cube_);
        if (!released) {
          release_guard_cube_.clear();
          return;
        }
        if (world_->SimTime() < release_guard_until_) {
          released->SetWorldPose(release_guard_pose_);
          released->SetLinearVel(ignition::math::Vector3d::Zero);
          released->SetAngularVel(ignition::math::Vector3d::Zero);
        } else {
          released->SetLinearVel(ignition::math::Vector3d::Zero);
          released->SetAngularVel(ignition::math::Vector3d::Zero);
          gzmsg << "[cube_carry] physical release guard completed for "
                << release_guard_cube_ << "\n";
          release_guard_cube_.clear();
        }
      }
...
private:
  ...
  rclcpp::Subscription<std_msgs::msg::Bool>::SharedPtr base_lock_sub_;
  ...
  bool base_locked_{false};
  ignition::math::Pose3d locked_base_pose_;
