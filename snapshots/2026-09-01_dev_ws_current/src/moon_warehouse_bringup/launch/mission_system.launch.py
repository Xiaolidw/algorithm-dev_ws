...
def generate_launch_description():
    ...
    rosapi = Node(
        ...
        name='rosapi',
        output='screen',
        # coStudio can query node details while the ROS graph is still being
        # populated.  Humble rosapi may throw on that transient empty result;
        # restart only this discovery helper without disturbing simulation.
        respawn=True,
        respawn_delay=2.0,
        parameters=[{
            'use_sim_time': use_sim_time,
            ...
            'topics_glob': (
                ...
                '/system/status,/system/status_text]'
            ),
        }],
        condition=IfCondition(start_rosbridge),
    )
    ...
    observability = Node(
        ...
        executable='runtime_observability_node.py',
        name='runtime_observability_node',
        output='screen',
