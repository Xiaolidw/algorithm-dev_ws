...
def generate_launch_description():
    ...
    # Gazebo needs several seconds to finish loading the large competition
    # world.  Spawning immediately can enqueue the robot successfully while
    # the factory service still times out waiting for it to become visible,
    # producing a false failure and racing controller startup.
    ...
    # Start all three spawners independently after the robot factory request.
    # Gazebo can create the entity and controller_manager but fail to deliver
    # the spawn service response; in that case spawn_entity.py remains alive
    # and an OnProcessExit dependency would prevent every controller loading.
    # Each spawner still waits up to 30 seconds for controller_manager.
    load_controllers = TimerAction(
        period=12.0,
        actions=[joint_state, arm, gripper],
    )
