...
class MissionFlowExecutorNode(Node):
    ...
    def __init__(self):
        ...
        # A pickup transit is only a collision-clear staging waypoint.  It
        # must not require terminal yaw or steal time from the actual
        # pre-approach / precision-dock gates that protect the cube.
        self.pickup_transit_acceptance_m = float(
            self.config.get('pickup_transit_acceptance_m', 0.15)
        )
        ...
        # Keep this timer for the node lifetime.  Humble's multi-threaded
        # executor can already have a timer in its wait set when its callback
        # cancels or destroys it; either operation can terminate executor.spin.
        # State guards below make the idle callback a no-op.
    ...
    def handoff_pickup_transit_proximity(self, position_error):
        ...
        The transit exists solely to put the base into a clear aisle before
        the normal pre-approach.  Requiring Nav2 to finish its final yaw here
        caused the robot to wait within 11 cm of the surveyed waypoint until
        the goal-progress watchdog declared a false stall.  The following
        pre-approach and strict final dock remain responsible for alignment
        and the physical grasp gate.
        """
        if (not self.active or self.current_phase != 'PICKUP_TRANSIT'
                or self.state not in self.NAVIGATION_STATES):
            return False
        self.get_logger().info(
            f'Pickup transit accepted at {position_error:.3f} m; '
            'cancelling terminal-yaw refinement before pre-approach.')
        if self.navigation_cancel_client.service_is_ready():
            self.navigation_cancel_client.call_async(Trigger.Request())
        record = self.make_leg_record('SUCCEEDED_PROXIMITY')
        self.leg_history.append(record)
        ...
        self.set_state(
            'NAVIGATING_PICKUP',
            f'Pickup transit safely reached for {task["object_id"]}; '
            'waiting for navigation cancel handoff',
            timeout_sec=4.0,
        )
        self.cancel_retry_timer()
