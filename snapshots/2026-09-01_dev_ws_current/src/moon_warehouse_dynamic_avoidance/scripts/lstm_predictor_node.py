...
class LstmObstaclePredictor(Node):
    ...
    def __init__(self):
        ...
        self.normalization_drift_sigma = max(
            0.5, float(self.get_parameter(
                'normalization_drift_sigma').value))
        self.normalization_drift_count = 0
    ...
    def declare_node_parameters(self):
        ...
        self.declare_parameter('normalization_drift_sigma', 2.0)
    ...
    def build_model_input(self, history):
        ...
        # Detect a model/statistics mismatch before inference.  Consecutive
        # out-of-distribution inputs raise into the existing safe fallback.
        observed_mean = input_tensor.mean(dim=1, keepdim=True)
        mean_z = ((observed_mean - self.input_mean) /
                  self.input_std.clamp_min(1e-4)).abs().max().item()
        if mean_z > self.normalization_drift_sigma:
            self.normalization_drift_count += 1
        else:
            self.normalization_drift_count = 0
        if self.normalization_drift_count >= 3:
            self.get_logger().warning(
                f'Normalization drift: mean_z={mean_z:.2f}; '
                'switching this prediction to route-reflection fallback')
            raise RuntimeError('normalization_drift')
