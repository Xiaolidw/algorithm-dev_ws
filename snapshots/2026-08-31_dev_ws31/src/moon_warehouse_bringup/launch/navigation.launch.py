...
def generate_launch_description():
    ...
    # 货物方块(顶面0.105m)低于雷达扫描面(0.125m), 扫描层看不见它们,
    # 机器人会直接碾过。方块位置在 world 文件中固定, 因此以虚拟障碍层
    # 注入两个代价地图: 全局规划绕行, 局部控制器实施防碾压。
    cube_obstacle_map = Node(
    ...
    # 静态恒等 map->odom: 本仿真 Gazebo 与地图原点重合, 控制器里程计即
    # 确定性定位。真值定位器在高负载虚拟机上时间戳抖动会引发 controller
    # 的 transformPose 外推风暴(实测 200+ 次/轮), 反而瘫痪导航;
    # 碰撞漂移由自回波屏蔽+限速+物理加固预防, AMCL 仅诊断不广播。
    map_to_odom = Node(
        package='tf2_ros',
